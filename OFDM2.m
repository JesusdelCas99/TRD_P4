...
plot(abs(rho2))
xlabel('muestra (n)')
title('Funcion metrica de correlacion, Senal OFDM2')
...
max_OFDM1=max(NFFT_1,NFFT_2); %Vemos que el modo de trabajo es 8K
...
%Para el fichero OFDM1 hemos obtenido el que el modo es 8k

%% Estimación de la duración del intervalo de guarda en DVB-T

%Numero de portadoras en 8k
NFFT_8K=8192;
...
[Ng_est,thr,rho_max] = guard_interval(Nd,rho2,NFFT_8K);

%Graficamos las funciones
figure(2)
plot(abs(rho2)/rho_max)
hold on
plot(thr)
hold off
grid on;
...
len_OFDM=NFFT_8K+Ng_est; %Longitud del simbolo OFDM
...
for i=0:len_OFDM-1
    suma=0;
    for j=0:Ng_est-1
        suma=conj(data(j+i+1))*data(j+i+NFFT_8K+1)+suma;
    end
    rho(i+1)=(suma)/Nw;
...
while((ini_n+(n_sim-1)*(NFFT_8K+Ng_est)<=l_archivo) && (ini_n+(n_sim-1)*(NFFT_8K+Ng_est)+NFFT_8K-1<=l_archivo))
    if n_sim==1
        sim_vec=data(ini_n:ini_n+NFFT_8K-1);
        ini_pos=ini_n;
        end_pos=ini_n+NFFT_8K-1;
    else
        sim_vec=vertcat(sim_vec,data(ini_n+(n_sim-1)*(NFFT_8K+Ng_est):ini_n+(n_sim-1)*(NFFT_8K+Ng_est)+NFFT_8K-1));
        ini_pos=horzcat(ini_pos,ini_n+(n_sim-1)*(NFFT_8K+Ng_est));
        end_pos=horzcat(end_pos,ini_n+(n_sim-1)*(NFFT_8K+Ng_est)+NFFT_8K-1);
    end
    n_sim=n_sim+1;
end
